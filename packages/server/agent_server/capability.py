...
import json
import logging
from typing import Dict, Any, Optional, Callable, Union, Iterator, List, Type, get_type_hints
import asyncio
import uuid
...
class CapabilityMetadata:
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        if self.expected_output_variability:
            behavioral_metadata["expectedOutputVariability"] = self.expected_output_variability
    ...
    def __init__(
        ...
        **kwargs
        ...
        if metadata:
            ...
            self.metadata = CapabilityMetadata(name=name, description=description, **kwargs)
        
        # Determine if function is async
        self.is_async = asyncio.iscoroutinefunction(func)
        
        # Create input validation model if schema provided
        self.input_model = self._create_input_model() if self.metadata.input_schema else None
    ...
    def _create_input_model(self) -> Type[BaseModel]:
        ...
        try:
            ...
            for field_name, field_schema in properties.items():
                field_type = self._schema_type_to_python(field_schema.get("type", "string"))
