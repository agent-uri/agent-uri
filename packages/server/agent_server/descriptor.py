...
import logging
import os
from typing import Dict, Any, List, Optional, Set, Callable, Type, Union

try:
    from agent_descriptor.models import AgentDescriptor
    from agent_descriptor.validator import validate_descriptor
except ImportError:
    # Fallback to base dict if agent-descriptor package not available
    AgentDescriptor = Dict[str, Any]

from agent_server.exceptions import DescriptorError
...
class AgentDescriptorGenerator:
    ...
    def scan_object_for_capabilities(self, obj: Any) -> int:
        ...
        for attr_name in dir(obj):
            ...
            if hasattr(attr, '_capability') and isinstance(attr._capability, Capability):
