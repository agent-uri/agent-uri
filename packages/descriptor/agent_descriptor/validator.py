...
def validate_required_fields(descriptor_data: Dict[str, Any]) -> ValidationResult:
    ...
    for i, capability in enumerate(capabilities):
        ...
        if "name" not in capability:
            result.add_error(f"capabilities[{i}]", "Capability missing required field: name")
        ...
        for i, skill in enumerate(skills):
            ...
            for field in ["id", "name"]:
                if field not in skill:
                    result.add_error(f"skills[{i}]", f"Skill missing required field: {field}")
    
    # Check if authentication contains required fields
    if "authentication" in descriptor_data and isinstance(descriptor_data["authentication"], dict):
        auth = descriptor_data["authentication"]
        if "schemes" not in auth:
            result.add_error("authentication", "Authentication missing required field: schemes")
        elif not isinstance(auth["schemes"], list):
            result.add_error("authentication.schemes", "Authentication schemes must be an array")
...
def _validate_type(value: Any, expected_type: Union[str, List[str]], 
...
def _validate_against_schema(value: Any, schema: Dict[str, Any], 
                           path: str, result: ValidationResult) -> None:
    """
...
def validate_agent_card_compatibility(descriptor_data: Dict[str, Any]) -> ValidationResult:
    """
    ...
    if "capabilities" in descriptor_data and isinstance(descriptor_data["capabilities"], dict):
        capabilities = descriptor_data["capabilities"]
