...
from agent_transport.base import (
    AgentTransport, TransportError, TransportTimeoutError
)
...
class LocalAgentRegistry:
    ...
    def register_agent(
        ...
        if not socket_path:
            ...
            if sys.platform.startswith('win'):
                ...
                socket_name = f"agent_{name.replace('/', '_')}_{uuid.uuid4().hex[:8]}.sock"
    ...
    def _start_agent_server(self, name: str) -> None:
        ...
        thread = threading.Thread(
            target=self._agent_server_loop,
            args=(name, server_socket),
            daemon=True
        )
        thread.start()
        ...
        if local_handler:
            ...
            try:
                ...
                raise TransportError(
                    f"Error streaming from local agent: {str(e)}"
                )
    ...
    def _send_request(
        ...
        try:
            ...
            if 'error' in response:
                raise TransportError(
                    f"Local agent error: {response['error']}"
                )
            ...
            while not complete:
                ...
                while b"\n" in buffer:
                    ...
                    try:
                        ...
                        if 'error' in data:
                            raise TransportError(
                                f"Local agent error: {data['error']}"
                            )
