...
from agent_transport.base import AgentTransport, TransportNotSupportedError
...
class TransportRegistry:
    ...
    def register_transport(
        self, transport_class: Type[AgentTransport]
    ) -> None:
        """
        ...
        if protocol in self._transports:
            logger.warning(
                f"Overriding existing transport for protocol '{protocol}'"
            )
    ...
    def unregister_transport(self, protocol: str) -> bool:
        ...
        if protocol in self._transports:
            ...
            logger.debug(
                f"Unregistered transport for protocol '{protocol}'"
            )
            return True
        return False
    ...
    def get_transport(self, protocol: str) -> AgentTransport:
        ...
        if protocol == "agent" and "https" in self._transports:
            logger.debug(
                "Using https transport as fallback for agent:// URI"
            )
