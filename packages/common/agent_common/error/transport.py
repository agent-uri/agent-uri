...
def parse_http_error(
    ...
    if "application/problem+json" in content_type:
        ...
        return AgentProblemDetail(
            ...
            extensions={k: v for k, v in data.items() 
...
def parse_websocket_error(data: Union[str, Dict[str, Any]]) -> Optional[AgentProblemDetail]:
    """
    ...
    if "error" in data:
        ...
        if isinstance(error_data, dict):
            ...
            return AgentProblemDetail(
                ...
                extensions={k: v for k, v in error_data.items() 
                             if k not in ["type", "title", "status", "detail", "instance"]}
            )
