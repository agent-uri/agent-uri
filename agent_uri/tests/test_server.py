...
from ..server import AgentServer, FASTAPI_AVAILABLE
...
class TestFastAPIAgentServer:
    ...
    def test_fastapi_server_initialization(self):
        ...
        server = FastAPIAgentServer(
            name="test-agent",
            version="1.0.0",
            description="Test FastAPI agent"
        )
    ...
    async def test_handle_http_request_get(self):
        ...
        with patch.object(server._handlers["http"], "handle_request", new_callable=AsyncMock) as mock_handler:
    ...
    async def test_handle_websocket_request(self):
        ...
        with patch.object(server._handlers["websocket"], "handle_request", return_value=mock_chunks()):
            chunks = []
            async for chunk in server.handle_websocket_request(
                path="/test",
                params={"param": "value"}
    ...
    async def test_websocket_request_error_handling(self):
        ...
        with patch.object(server._handlers["websocket"], "handle_request", return_value=mock_error_handler()):
...
class TestFastAPIUnavailable:
    ...
    def test_fastapi_server_import_error(self):
        """Test that FastAPIAgentServer raises ImportError when FastAPI is not available."""
...
class TestServerIntegration:
    ...
    def test_authenticator_registration_and_propagation(self):
        ...
        with patch.object(server._handlers["http"], "register_authenticator") as mock_http_auth:
            with patch.object(server._handlers["websocket"], "register_authenticator") as mock_ws_auth:
                server.register_authenticator(auth_func)
    ...
    def test_capability_registration_with_handlers(self):
        ...
        # Mock handler registration
        with patch.object(server._handlers["http"], "register_capability") as mock_http_cap:
            with patch.object(server._handlers["websocket"], "register_capability") as mock_ws_cap:
