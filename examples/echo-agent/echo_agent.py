...
async def echo(
    ...
    **kwargs,
    ...
    logger.info(
        f"Echo capability called with params: {params}, session_id: {session_id}"
    )
    if kwargs:
    ...
    return {"result": result, "timestamp": current_time, "original_message": message}
...
def main():
    ...
    )  # Changed default port to avoid conflicts
